...
<head>
  ...
  <title>MadinaTech - Signalés</title>
  ...
  <div id="wrapper">
      ...
      <div id="content">
        <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow justify-content-between">
          ...
          <form action="" method="post" class="form-inline my-2 my-lg-0">
            <input class="form-control mr-sm-2" id="search" type="search" name="username" placeholder="Nom d'utilisateur" aria-label="Search">
   ...
   if (!empty($_POST['username'])) {
     ...
     while ($row = mysqli_fetch_array($r_query)){
       ...
       echo '<form action="signaledusers.php" method="post">
       <input type="hidden"  value="' . $row['id']  . '" name="hid">
       <input type="hidden"  value="' . $row['email']  . '" name="mail">
       <input  class="btn btn-danger" type="submit" value="Désactiver profile" name="supprimer">
       </form>';
       echo '</div>';
       echo '</div>';
       echo '</div>';
   ...
   $result = mysqli_query($db,"SELECT * FROM user WHERE visible=1 and signaled<>0 order by signaled desc");
   ...
   if(isset($_POST['supprimer'])){
       ...
       if($requete){
         ...
         print("<script type=\"text/javascript\">location.href=\"signaledusers.php\"</script>");
       }
       else{
