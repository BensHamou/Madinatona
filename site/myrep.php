...
<body id="default_theme" class="services" style="background-color: C9FFE5;">
                ...
                <div class="logo" data-toggle="tooltip" title="Allez à la page d'accueil!" style="width:20px"><a href="home.php"><img src="images/logo.png" style="width:120px;height:80px" alt="#" /></a></div>
                 <div class="main_menu float-right">
                    <div class="menu" style="background-color: C9FFE5;">
                      ...
                      <a class="nav-link active dropdown-toggle" style="background-color:LightBlue" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" href="myrep.php">Tous</a>
    ...
    $result = mysqli_query($db,"SELECT * FROM signalement where username = '".$user."' and signalement.visible!=0 order by date1 desc");
    while($row = mysqli_fetch_array($result)){
      echo '<div class="col-md-6" style="margin-top:5%;">';
      if($row['etat']==1){
        echo '<div class="card border-danger" style="margin-top:5%;margin-bottom:2%;">';
      }
      else if($row['etat']==2){
        echo '<div class="card border-warning" style="margin-top:5%;margin-bottom:2%;">';
      }
      else if($row['etat']==3){
        echo '<div class="card border-success" style="margin-top:5%;margin-bottom:2%;">';
      }
      ...
      if($row['etat']==1 && $row['visible']==1 && empty( $row['rapport'])){
        echo '<form action="deleterep.php" onsubmit="return confirm(\'Tu es sure que tu veux supprimer ce signalement?\');" method="POST">';
        echo '<button type="submit" class="btn btn-danger" style="margin-left:35%;margin-top:15%;" name="delete"  value="' . $row['id']  . '">Supprimer</button>';
        echo '</form>';
      }
      echo '</div>';
      if($row['etat']==1){
        if($row['visible']==1){
          echo  '<p5 class="adress" style="color:red;margin-left:auto;margin-right:auto;">Signalement pas encore validé.</p>';
        }
        else
        echo  '<p5 class="adress" style="color:red;margin-left:auto;margin-right:auto;">Signalement pas encore traité.</p>';
      }
      else if($row['etat']==2){
        echo  '<p5 class="adress" style="color:orange;margin-left:auto;margin-right:auto;">Signalement mise en prise.</p>';
      }
      else if($row['etat']==3){
        echo  '<p5 class="adress" style="color:green;margin-left:auto;margin-right:auto;">Signalement cloturé.</p>';
      }
      echo '</div>';
      echo '</div>';
    }
